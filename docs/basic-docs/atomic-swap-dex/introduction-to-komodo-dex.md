...
| An exchange can be set up by anyone | Check: Only two users and an Internet connection are required, with minimal legal clearance | X-cross: Requires sufficient technical knowledge and funding, and some legal clearnace | Partial check: Requires sufficient technical knowledge and funding, and a strong amount of legal resources | 
| Users maintain control over the funds throughout the process | Check: Users maintain control over their funds until the trade is completed, at which point ownership is swapped atomically | X-cross: Users relenquish ownership to the gateway nodes, although users do hold private keys to IOUs | X-cross: Once the user sends funds to the CEX, they only own their funds in theory, until the actual funds are returned |
